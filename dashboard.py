...
st.subheader('Monthly Orders Distribution in 2018')
...
ax.plot(
    ...
    marker='o',
    linewidth=2,
    color="#72BCD4",  # Sesuaikan warna dengan notebook
)
...
st.subheader('Total Customer Expenditure in 2018')
...
sns.barplot(
    ...
    palette=["#FF5733", "#FFD700", "#C0C0C0", "#808080", "#000000"],  # Sesuaikan warna dengan notebook
    ax=ax[0],
)
ax[0].set_ylabel('')
ax[0].set_xlabel('')
...
sns.barplot(
    ...
    palette=["#FF5733", "#D3D3D3", "#D3D3D3", "#D3D3D3", "#D3D3D3"],  # Sesuaikan warna dengan notebook
    ax=ax[1],
)
ax[1].set_ylabel('')
ax[1].set_xlabel('')
...
rating_service, max_rating, ratings = customers_rating_df(df_all_data)
plt.figure(figsize=(10, 5))
sns.barplot(x=rating_service.index,
            y=rating_service.values,
            order=rating_service.index,
            palette=["#FF5733" if rating == max_rating else "#D3D3D3" for rating in rating_service.index] 
            )
plt.title("Customer Ratings for Service", fontsize=15)
plt.xlabel("Rating")
